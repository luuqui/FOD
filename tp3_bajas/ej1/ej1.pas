...
procedure eliminar(var arc: archivo_emp);
var
	emp, aux: empleado;
	nro, posBorrar: integer;
	flag: boolean;
begin
	flag:= false;
	writeln('Ingrese el nro de empleado a eliminar');
	readln(nro);
	//la bandera ayuda en caso de que el archivo venga sin registros, ya que si hago un read antes del while y no hay registros tira error de ejecucion.
	while(not EOF(arc)and(not flag))do begin
		read(arc, emp);
		if(emp.nro = nro)then
			flag:= true
	end;
	...
	if(flag)then begin
		...
		writeln('No se encontro el empleado o el archivo está vacio');
end;
